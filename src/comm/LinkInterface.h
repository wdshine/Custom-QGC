...
// LinkInterface类：
//  1. 通信链路抽象 作为所有通信链路（Serial/UDP/TCP等）的基类，提供统一的接口规范：
//  2. MAVLink 通道管理 通过 LinkManager 分配 MAVLink 通道号，实现多链路并行通信：
//  3. 多协议支持 通过继承实现不同通信方式：
//     - SerialLink 处理串口通信（波特率/流控等）
//     - UDPLink 实现组播通信
//     - TCPLink 提供可靠连接
//     - LogReplayLink 支持日志回放
//  4. 生命周期管理 与 LinkManager 协同工作，通过引用计数管理链路生命周期：
//  5. 车辆关联管理 通过引用计数机制确保多车辆场景下的资源安全：
//  6. 高延迟链路支持 通过配置标记区分常规/高延迟链路，优化通信策略：
 
//  典型工作流程：
//     1. LinkManager 创建具体链路实例
//     2. 分配 MAVLink 通道号
//     3. 连接底层通信接口
//     4. 转发数据到 MAVLinkProtocol
//     5. 通过 VehicleLinkManager 实现车辆级通信管理
